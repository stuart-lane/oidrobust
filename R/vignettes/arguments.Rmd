---
title: "Function arguments and basic usage"
author: "Stuart Lane"
date: "`r Sys.Date()`"
...
- `errors`:
  ...
  - If specified, must be one of these options:
    ...
    - `"hac"` (Newey-West heteroskedasticity- and autocorrelation-robust variance estimator)
...
If `errors = "hac"`, then there is an additional optional argument:
  ...
  - `lags`:
    ...
    - Available preset options to automatically calculate number of lags are:
      - `"rot"` (rule-of-thumb)
      - `"plug-in"` (plug-in)
    - Alternatively, the user can specify any non-negative integer `"L"`
      - If `"L=0"`, this collapses to the White heteroskedasticity-robust variance estimator
...
#### Additional arguments
  ...
  - `no_constant`:
    ...
    - Optional, default is `NULL`
    - Must be either `TRUE`, `FALSE` or `NULL`
    - Note: must be explicitly set to `"TRUE"` if constant is not wanted in the regression
