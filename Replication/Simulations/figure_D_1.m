...
for Kj = 1:length(Kvals)
    ...
    for rhoj = 1:length(rhovals)
        ...
        graph = figure
        p31 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,1,3),K-2,'upper')<0.05),'b')
        hold on
        p32 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,2,3),K-2,'upper')<0.05),'b--')
        hold on
        p33 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,3,3),K-2,'upper')<0.05),'b:')
        hold on
        p34 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,1,4),K-2,'upper')<0.05),'r')
        hold on
        p35 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,2,4),K-2,'upper')<0.05),'r--')
        hold on
        p36 = plot(mu2a,mean(chi2cdf(sb(:,mu2k,3,4),K-2,'upper')<0.05),'r:')
        hold on
        ...
        legend('$J$ $(\alpha = 0.05)$','$J$ $(\alpha = 0.1)$','$J$ $(\alpha = 0.2)$','$KP$ $(\alpha = 0.05)$','$KP$ $(\alpha = 0.1)$','$KP$ $(\alpha = 0.2)$','Location','northeast','Interpreter','Latex')
